...
class TSP:
    ...
    @classmethod
    def optimize_solution(cls, distances, connections):
        """Tries to optimize solution, found by the greedy algorithm"""
        N = len(connections)
        path = cls.restore_path(connections)
    ...
    @classmethod
    def restore_path(cls, connections):
    ...
    @classmethod
    def pairs_by_dist(cls, N, distances):
    ...
    @classmethod
    def solve_tsp(cls, distances, optim_steps=3):
        ...
        join_segments(cls.pairs_by_dist(N, distances))

        for passn in range(optim_steps):
            nopt, dtotal = cls.optimize_solution(distances, connections)
            if nopt == 0:
                break
