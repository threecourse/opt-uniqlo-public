...
if __name__ == "__main__":
    ...
    """
    if Train:
        x2 = aug.transform_batch_train_image(x)
        x2r = preprocess_input_reverse(x2)
        for ii in range(x2r.shape[0]):
            xx = x2r[ii]
            fig, axes = plt.subplots(1, 5, figsize=(16, 4))
            axes[0].imshow(xx / 255.0)
            plt.show()

    if Test:
        x2 = aug.transform_batch_test_image(x, n=5)
        print x2.shape

        x2r = []
        for i in range(x2.shape[0]):
            x2r.append(preprocess_input_reverse(x2[i, :]))
        x2r = np.array(x2r)
    ...
    if Train:
        for ii in range(x.shape[0]):
            xx = aug.transform_train_image(x0[ii])
            fig, axes = plt.subplots(1, 5, figsize=(16,4))
            axes[0].imshow(xx / 255.0)
            plt.show()
    ...
    if Test:
        for ii in range(x.shape[0]):
            x_list = aug.transform_test_images(x0[ii], 5)
            fig, axes = plt.subplots(1, 5, figsize=(16,4))
            for i, xx in enumerate(x_list):
                axes[i].imshow(xx / 255.0)
            plt.show()
    """
