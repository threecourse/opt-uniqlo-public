...
class ModelKerasCNN5_4(ModelBase):

    def _identity_block_1x1(self, input_tensor, filters, stage):
    ...
    def _get_model_structure(self, i_bag):
        ...
        ch2 = self.prms["ch2"]
        k2 = self.prms["k2"]
        strides2 = self.prms["strides2"]
        dropout2 = self.prms["dropout2"]

        d1 = self.prms["d1"]
        dropout3 = self.prms["dropout3"]

        x = Conv2D(ch1, kernel_size=(k1, k1))(inputs)
        x = Activation('relu')(x)
        x = Conv2D(ch1, kernel_size=(k1_2, k1_2), strides=(strides1, strides1))(x)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        for i in range(self.prms["iblocks"]):
            x = self._identity_block_1x1(x, filters=[64, ch1], stage=i)
        x = MaxPooling2D(pool_size=(2, 2))(x)
        x = Dropout(dropout1)(x)

        x = Conv2D(ch2, kernel_size=(k2, k2))(x)
        x = Activation('relu')(x)
        x = Conv2D(ch2, kernel_size=(k2, k2), strides=(strides2, strides2))(x)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        x = MaxPooling2D(pool_size=(2, 2))(x)
